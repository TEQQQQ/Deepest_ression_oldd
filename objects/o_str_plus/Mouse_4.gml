...
if global.character == 4 {
	if o_rogue.stat_points >= 1 {
		...
		//10+%3
		o_rogue.attack += 10
		//o_rogue.attack += 10+(o_rogue.attack*3)/100;
...
if global.character == 3 {
	if o_mage.stat_points >= 1 {
		...
		//15+%1
		o_mage.attack += 15+(o_mage.attack*1)/100;
	}
}
...
if global.character == 1 {
	if o_knight.stat_points >= 1 {
		...
		//30+%4
		o_knight.attack += 30+(o_knight.attack*4)/100;
	}
}
...
if global.character == 2 {
	if o_priest.stat_points >= 1 {
		...
		//15+%3
		o_priest.attack += 15+(o_priest.attack*3)/100;
	}
}
