...
if room == Room_info or room == Room1 {
	...
	if position_meeting(mouse_x,mouse_y,o_rogue_belt_lichking_inv2){
			...
			if hareket == false {
				if mouse_check_button_pressed(mb_right){
					if in_equipment == false {
						if o_rogue.equipment_belt == 0 {
							o_rogue.equipment_belt = 1;
							in_equipment = true;
	...
	if hareket == false and global.mouse_item == 0 {
		...
		if position == 24 {
			x = o_inventory_place_belt.x;
			y = o_inventory_place_belt.y;	
		}
	}
