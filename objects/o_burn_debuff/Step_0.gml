...
*/
/*
if (o_buff_system.player_buff_count-previous_count) > 3 {
	x = o_buff_system.x+((o_buff_system.player_buff_count-previous_count)-4)*24;
	y = o_buff_system.y+24;
	if (o_buff_system.player_buff_count-previous_count) > 7 {
		x = o_buff_system.x+((o_buff_system.player_buff_count-previous_count)-8)*24;
		y = o_buff_system.y+48;
		if (o_buff_system.player_buff_count-previous_count) > 11 {
			x = o_buff_system.x+((o_buff_system.player_buff_count-previous_count)-12)*24;
...
if once == true {
	if !instance_exists(o_burn_effect){
		instance_create_layer(o_border_player.x,o_border_player.y,"effects",o_burn_effect);	
		once = false;
	}
}
