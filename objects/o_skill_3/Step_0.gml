			...
			if position_meeting(mouse_x,mouse_y,o_skill_3){
				if hareket == false {
					if sprite_index != s_skill_1 {
						if !instance_exists(o_hint_skill_info_3){
							instance_create_layer(x,y,"hints",o_hint_skill_info_3);	
						}
					}
				} else {
					instance_destroy(o_hint_skill_info_3);
				}
			} else {
				instance_destroy(o_hint_skill_info_3);
			}
						...
						if o_rogue.critical_chance <= o_rogue.critical_rate {
							if o_tier1_1_bandicot.dodge_chance >= o_tier1_1_bandicot.dodge_rate {
								if (2*(ceil(o_rogue.attack)+ceil(o_rogue.sapma)-o_tier1_1_bandicot.armor) > 0) {
									if !instance_exists(o_crit_info_3) {
										instance_create_layer(x,y,"buttons",o_crit_info_3);
									}
